...
const writeJSON = (filename, obj) => fs.outputJSON(filename, obj, { spaces: 2 })
...
async function downloadGame(dir, gameId) {
  ...
  await writeJSON(resolve(gameDir, 'comments.json'), await api.getGameComments(gameId))
...
async function downloadGamesFromCompanies(dir, companies) {
  const games = companies.reduce((games, company) => games.concat(company.games), [])

  for (const i in games) {
    const game = games[i]
    debug(`downloading game ${i}/${games.length}`)
    try {
      await downloadGame(dir, game.id)
    } catch (err) {
      debug('err', game.id, err.message)
    }
  }
}
module.exports = {
